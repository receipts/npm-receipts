...
var getLotteryTradesCollection = function getLotteryTradesCollection(id, callback) {
  ...
  trades.push(
    new TradeResponseBuilder()
      .withId(Trade.FOOD_AND_DRINKS)
      .withName('Restauracje i inne placówki gastronomiczne oraz przygotowanie i podawanie napojów')
      .build()
  );
  trades.push(
    new TradeResponseBuilder()
...
var getLotteryResultsCollection = function getLotteryResultsCollection(id, options, callback) {
  ...
  return results.getLotteryResultsCollection(options, function (err, collection) {
    if (err) {
      logger.error(err);
      return callback(err);
    }
