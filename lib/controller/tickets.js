...
var parseUserAuth = function parseUserAuth(string, next) {
  ...
  if (!decoded) {
    return next(new UnauthorizedError('UserAuth header invalid'));
  }
  ...
  receipts.createLotteryTicket(lotteryId, ticketRequest, options, function (err, data) {
    ...
    if (json.hasOwnProperty('success')) {
      ...
      if (json.hasOwnProperty('nr_kasy')) {
        return next(new BadRequestError('Invalid pointOfSale number', json.nr_kasy.join(','), 'pointOfSale'));
      }

      if (json.hasOwnProperty('miesiac')) {
        return next(new BadRequestError('Invalid date', json.miesiac.join(','), 'date'));
      }

      if (json.hasOwnProperty('kwota_zl')) {
        return next(new BadRequestError('Invalid amount value', json.kwota_zl.join(','), 'amount.value'));
      }

      if (json.hasOwnProperty('nip')) {
        if (json.nip.length > 1) {
          return next(new BadRequestError('Invalid taxRegistrationNumber number', json.nip[1], 'taxRegistrationNumber'));
        }
        else {
          return next(new BadRequestError('Invalid taxRegistrationNumber number', json.nip[0], 'taxRegistrationNumber'));
        }
      }
    }
